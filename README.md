...
SlowLink Assistant Bot 与主 [SlowLink](https://github.com/suzijin876-lgtm/slowlink) 分开运行。它不负责内容识别、规则匹配或去重，只负责频道消息复制、主人私聊通知，以及向固定群组和可选频道发送日报、周报和月报。
...
| 权限限制 | 私聊与群按钮仅允许配置的主人使用，非授权群组和频道可自动退出 |
| SQLite 记录 | 保存更新偏移、转发结果、统计覆盖时间、目标投递状态和发送快照 |
| 稳定运行 | Docker 自动重启、主循环心跳、数据库备份、日志轮换和独立 watchdog |
...
| 运行状态 | 查看版本、运行时间及目标权限 |
| 最近记录 | 查看最近转发结果 |
| 系统自检 | 检查 Telegram、数据库和 watchdog |
| 简报设置 | 分别开关群和频道的日报、周报、月报 |
| 封面管理 | 设置、预览或停用定时报表封面 |

旧版私聊命令继续兼容，但不再显示在 Telegram 命令列表中。

回复Bot转发的私聊消息发送`删`、`删除`或`已删除`，可处理或记录对应源频道帖子；私聊副本会保留。

在主面板点击“封面管理”和“更换封面”，随后直接发送图片即可设置；Bot会在私聊发送真实数据预览。

固定报表群仅提供主人可用的“当前报告”按钮。
