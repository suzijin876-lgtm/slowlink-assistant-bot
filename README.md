...
| 定时报表 | 每日、每周和每月自动发送有效转发、周期环比和活跃统计 |
| 固定报表封面 | 主人上传一次后复用于日报、周报和月报，统计文字仍会动态更新 |
...
主人私聊发送图片并在图片说明中填写`/cover`，可设置或更换日报、周报和月报的固定封面；发送`/cover off`可停用，发送`/cover`可查看状态。该管理命令不会出现在Bot公开帮助中。
