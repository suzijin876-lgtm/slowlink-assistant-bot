...
| 定时报表 | 每日、每周和每月自动发送有效转发、完整同期变化和活跃统计，错过设定分钟后会在触发日补发 |
| 固定报表封面 | 主人上传一次后复用于日报、周报和月报，并在主人私聊生成真实数据预览 |
| 权限限制 | 私聊和群组命令仅允许配置的主人使用，非授权群组可自动退出 |
| SQLite 记录 | 保存更新偏移、转发结果、统计覆盖时间、报表状态和发送快照 |
...
主人私聊发送图片并在图片说明中填写`/cover`，可设置或更换日报、周报和月报的固定封面；设置后Bot会在私聊发送一份真实数据预览。发送`/cover off`可停用，发送`/cover`可查看状态。该管理命令不会出现在Bot公开帮助中。
